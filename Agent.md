...
9. 用户确认图片资产或外部生成准备完成后，触发 `shot_video_prompt_generator`，输入分镜、资产提示词、视觉风格、分镜图与参考素材，产出 `outputs/05_video_prompts/shot_video_prompts.md`。视频提示词只输出中文。
10. 用户触发 `external_generation_handoff`，把逐镜头中文视频提示词、分镜图、人物/场景引用和剪辑说明整理成可复制到外部网页端的交接包。
11. 用户将视频提示词复制到即梦/Seedance 网页端或其他视频平台生成视频。
12. 用户可把外部生成结果的文件名、链接、截图说明或人工备注记录到本地 run 目录；这些结果不进入仓库。
13. 用户触发 `continuity_review`，输入故事、风格、分镜、资产、视频提示词和可选的生成结果备注，产出 `outputs/07_final_delivery/continuity_report.md`。
14. 剪辑在剪映等外部工具完成，CodeX 只输出剪辑交接清单，不执行剪辑。
15. 用户确认全部产物后，触发 `production_package_builder`，汇总最终交付包。
...
- `shot_video_prompt_generator` 必须引用已确认资产 ID，不得凭空重建角色或场景；默认只 `@` 分镜图、人物资产和场景资产，道具只写入画面描述。
