...
5. 用户确认视觉方向后，触发 `storyboard_director`，产出 `outputs/storyboard.json`。导演负责镜头结构化，但不得输出资产草表、资产 ID 或音色定义。
6. 用户确认分镜序列后，触发 `asset_executor`，产出 `outputs/asset_manifest.json` 与 `outputs/shot_asset_map.json`。资产执行官负责人物、场景、道具和资产拆分。
...
11. 触发 `video_prompt_generator`，产出 `outputs/video_prompts.md` 与 `outputs/video_prompts.json`。
