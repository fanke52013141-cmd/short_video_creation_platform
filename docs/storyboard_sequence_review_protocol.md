...
除了传统连续性检查，本协议必须审查两个硬规则：

1. **单镜头时长不得超过 15 秒。**
2. **分镜必须是真实镜头边界，不得把一个连续镜头硬拆成两份。**

原因：AI 视频生成通常按 shot 独立生成。若把同一机位、同一景别、同一空间、同一连续动作硬拆成多个 shot，生成结果很容易出现角色、场景、光线、道具位置和运动方向不一致。

## 审查窗口

逐 shot 审查不足以发现连续性问题。必须使用滑动窗口：

- `1-shot`: 检查单镜头内部是否自洽，包括时长、叙事功能、镜头边界理由。
- `2-shot`: 检查当前镜头与上一个镜头的直接承接，包括是否存在连续镜头硬拆。
- `3-shot`: 检查当前镜头与前后镜头组成的小段落逻辑。
- `sequence`: 检查全片母题、时间线、人物状态、空间规则和节奏曲线。
  ...
  "issues": [
    {
      ...
      "category": "duration | shot_boundary | storytelling | space | prop | character | time | sound | motif | style",
      "description": "",
      "fix_suggestion": ""
    }
  ]
...
- `P0`: 会造成明显穿帮、因果断裂、AI 生成片段无法接续，或违反硬规则。包括 shot 超过 15 秒、连续镜头硬拆、关键人物/场景/道具状态断裂。
- `P1`: 会削弱叙事清晰度或造成生成风险，建议修正。包括镜头叙事功能弱、边界理由不足、相邻镜头信息重复。
