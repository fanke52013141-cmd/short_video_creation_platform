...
- 视频提示词资产引用：已补分镜图、人物、场景可 `@`，道具默认不 `@PROP` 的规则。
...
- `single_shot_exporter`：长片项目需要把 `shot_video_prompts.md` 拆成逐 shot 文件，方便复制到外部工具。

## MVP 结论
当前提示词已经足够支撑第一版流程。必须新增的是三个胶水层：
1. `asset_manifest_builder`
2. `continuity_review`
3. `external_generation_handoff`
4. `production_package_builder`
