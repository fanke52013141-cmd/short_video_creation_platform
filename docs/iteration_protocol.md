...
- 某个流程阶段顺序不合理，需要更新 README、`docs/flow.md` 和 `config/config.yaml`。
...
| 问题类型 | 优先修改文件 |
|---|---|
| 阶段顺序、分支选择、人工确认点 | `README.md`, `docs/flow.md`, `config/config.yaml`, `checkpoint.template.json` |
| Skill 输入输出边界 | `skills/*.md`, `skills/raw_prompts/*.source.md`, `config/config.yaml` |
| 视频提示词格式、只输出中文、资产声明规则 | `skills/video_prompt_generator.md`, `skills/raw_prompts/seedance_video_prompt.source.md`, `schemas/video_prompt.schema.json` |
| 分镜提示词帧角色和站位锚点 | `skills/storyboard_prompt_generator.md`, `docs/flow.md`, `scripts/validate_project.py` |
| 人物、场景、道具资产命名 | `skills/asset_executor.md`, `schemas/asset_manifest.schema.json`, `schemas/shot_asset_map.schema.json` |
| 内部/外部图片生成分支 | `skills/image_generation_executor.md`, `docs/generation_mode_protocol.md`, `config/config.yaml`, `checkpoint.template.json` |
...
- [ ] 如果改了流程顺序，已更新 README、`docs/flow.md` 和 `config/config.yaml`。
