...
<DirectorMethod>
你必须先用以下方法内部分析，再输出最小 `storyboard.json`。这些分析结果不作为字段输出。
...
规则：
- 景别不是装饰，而是观众与人物的心理距离。
- 镜头运动不是炫技，而是注意力或情绪的运动。
- 时长不是填数字，而是观众消化信息所需时间。
- action_desc 必须是可见、可拍、可执行的动作和画面变化。
...
镜头 A + 镜头 B 不只是 A 然后 B；相接后必须产生新的观看意义。
</DirectorMethod>
...
禁止只写：很疲惫、氛围压抑、气氛紧张、关系疏离、很孤独。

必须落地为：微表情、肢体动作、视线、空间距离、声音、光影变化、物体状态、环境反应。
