...
## Purpose
生成最终可复制到即梦 / Seedance 2.0 的中文视频提示词文档 `video_prompts.md`。
...
| `multimodal_reference` | 使用图片、音频、视频作为参考维度生成新视频 | 使用 `参考@资产名` |
...
- `动作参考`：锁定运动模式、动作节奏和身体动态。
- `风格参考`：锁定视觉语言、剪辑节奏、色彩和质感，不锁定具体内容。必须写明：参考风格，不参考具体人物或事件。
- `整体参考`：锁定大部分画面逻辑、节奏和镜头语言。正文只写需要修改、替换或强化的部分。

#### 操作对象类视频
正文直接使用 `@资产名`，禁止加 `参考` 前缀：
...
- 多个参考素材在风格、色彩、构图、环境或人物状态上冲突时，必须显式写明“以某某素材为准”。
- 默认优先级：用户文字要求 > 首帧/尾帧/关键帧 > 人物资产 > 环境资产 > 风格参考 > 音频参考。
...
- 错误：小孩在打游戏。
- 正确：小孩侧坐在沙发边缘，双手握住手机，两个拇指在屏幕上高频滑动，屏幕蓝光在脸上闪烁，眼神始终盯着屏幕。

## Camera And Action Rules

### Single Camera Movement Rule
每个镜头只能指定一种运镜方式：推近、拉远、横移、摇镜、跟随、升降、环绕、固定，选其一。

禁止在同一镜头内叠加多种运镜，例如“缓慢推近的同时向右横移”。
