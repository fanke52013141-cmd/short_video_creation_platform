...
## Purpose
在分镜生成后，对全片分镜做镜头边界、单镜头时长、相邻镜头逻辑和全片连续性审查，发现会导致穿帮、空间跳变、道具凭空出现、人物状态断裂、声音来源不明、母题误用、镜头边界伪拆分和 AI 视频生成不一致的问题。
...
2. 读取故事、风格圣经和完整分镜。
3. 建立 shot 顺序表，包含每个 shot 的时长、场景、景别、人物、道具、声音、帧策略、镜头边界类型、镜头边界理由和画面提示词。
4. 对每个 shot 做单镜头自洽检查。
5. 对每个 shot 做时长检查：`duration_seconds` 必须 `> 0` 且 `<= 15`。
6. 对每个 shot 做镜头边界检查：确认它是独立镜头单位，而不是连续镜头硬拆。
7. 用 `SHOT_N-1 + SHOT_N` 做 2-shot 相邻检查。
8. 用 `SHOT_N-1 + SHOT_N + SHOT_N+1` 做 3-shot 小段落检查。
9. 检查全片母题、时间线、人物状态、道具位置和声音来源是否连续。
10. 给每个问题标记 `P0`、`P1` 或 `P2`。
11. 输出 Markdown 审查报告和 JSON 机器可读报告。
...
- `P1`: 会削弱叙事清晰度、镜头表达力或造成较高生成风险。包括：镜头叙事功能弱、边界理由不足、相邻镜头信息重复。
